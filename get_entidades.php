<?php
    ...
    try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            ...
            $cd_entidade = descriptografar($cd, $chave);
            //print('Texto Descriptografado: ' . $cd_entidade);
            // Ajusta a consulta SQL para filtrar por cd_entidade (se necessário)
            if ($cd_entidade > 0) {
                $stmt = $conn->prepare("SELECT cd_entidade, nome FROM entidade WHERE cd_entidade = :cd_entidade ORDER BY nome");
                $stmt->bindParam(':cd_entidade', $cd_entidade, PDO::PARAM_INT);
            } else {
                // Executa a consulta sem parâmetros desnecessários
                $stmt = $conn->prepare("SELECT cd_entidade, nome FROM entidade ORDER BY nome");
            }
        $stmt->execute();
