<?php
    ...
    try {
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            ...
            if ($cd_contato > 0) {
                $stmt = $conn->prepare("SELECT cd_contato, nome FROM contato WHERE cd_contato = :cd_contato ORDER BY nome");
                $stmt->bindParam(':cd_contato', $cd_contato, PDO::PARAM_INT);
            } else {
                // Executa a consulta sem parâmetros desnecessários
                $stmt = $conn->prepare("SELECT nome FROM contato ORDER BY nome");
            }
        $stmt->execute();
